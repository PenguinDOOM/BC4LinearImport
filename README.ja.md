...
VRChat のアバターやワールドでグレースケールテクスチャを使うときに、毎回外部ソフトウェアで変換する手間を減らしたい場合に役立ちます。
...
手動の再インポート経路、自動インポート時の経路、安全側の no-op をまとめて見たい場合は、[BC4LinearImport color-space flow diagram](docs/diagrams/bc4linearimport-color-space-flow.svg) を参照してください。
...
もし `0 eligible textures` と表示されたり、テクスチャが変わらなかったりした場合は、まず [トラブルシューティング](docs/troubleshooting.ja.md) を見るのがおすすめです。