...
手動の再インポートが同じ自動チェックへ戻る流れや、条件つきの BC4 対象判定、安全側の no-op を図で確認したい場合は、[BC4LinearImport color-space flow diagram](diagrams/bc4linearimport-color-space-flow.svg) を参照してください。
...
「なぜ変換されなかったの？」をもう少し詳しく確認したい場合は、[トラブルシューティング](troubleshooting.ja.md) を見てください。
